...
st.markdown(
    """
    <style>
    /* Compact iPhone layout pass: less scrolling and smaller controls */
    .block-container { padding-top: .55rem; padding-left: .45rem; padding-right: .45rem; padding-bottom: 3rem; max-width: 760px; }
    h1 { font-size: 1.38rem !important; line-height: 1.15 !important; margin-bottom: .25rem !important; }
    h2 { font-size: 1.14rem !important; line-height: 1.16 !important; margin-bottom: .20rem !important; }
    h3 { font-size: 1.02rem !important; line-height: 1.16 !important; margin-bottom: .18rem !important; }
    p, label, div, span { font-size: .93rem; }
    div[data-testid="stButton"] > button,
    div[data-testid="stFormSubmitButton"] > button,
    .stDownloadButton > button { width: 100%; min-height: 2.15rem; border-radius: .45rem; font-size: .86rem; font-weight: 650; padding: .15rem .20rem; margin: 0; white-space: nowrap; }
    div[data-testid="stMetric"] { padding: .42rem; border: 1px solid rgba(128,128,128,.14); border-radius: .55rem; margin-bottom: .20rem; }
    div[data-testid="stMetricValue"] { font-size: 1.08rem !important; }
    div[data-testid="stHorizontalBlock"] { gap: .14rem; margin-bottom: .04rem; }
    .compact-row { padding: .04rem 0 .05rem 0; border-bottom: 1px solid rgba(128,128,128,.14); line-height: 1.05; }
    .compact-product-name { font-weight: 700; font-size: .88rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; padding-top: .38rem; }
    .compact-price { font-size: .80rem; opacity: .82; text-align: right; padding-top: .38rem; white-space: nowrap; }
    .qty-box { text-align:center; min-height: 2.00rem; padding:.35rem 0 .18rem 0; font-size:1.00rem; border:1px solid rgba(128,128,128,.28); border-radius:.42rem; background: rgba(250,250,250,.035); }
    .small-caption { font-size:.78rem; opacity:.72; }
    .ez-card { padding: .50rem; border: 1px solid rgba(128,128,128,.20); border-radius: .60rem; margin: .25rem 0 .35rem 0; }
    div[data-testid="stDataFrame"] { margin-top: .20rem; }
    @media (max-width: 700px) {
        .block-container { padding-left: .30rem; padding-right: .30rem; max-width: 100%; }
        div[data-testid="stHorizontalBlock"] { gap: .08rem; }
        div[data-testid="stButton"] > button,
        div[data-testid="stFormSubmitButton"] > button,
        .stDownloadButton > button { min-height: 1.95rem; font-size: .80rem; padding-left: .08rem; padding-right: .08rem; }
        .qty-box { min-height: 1.92rem; font-size: .98rem; padding-top: .34rem; }
        .compact-product-name { font-size: .84rem; }
        .compact-price { font-size: .76rem; }
    }
    </style>
    """,
...
def render_product_entry():
    ...
    filter_text = st.text_input("Filter products / Buscar producto", key="product_filter", placeholder="Type to find an item...")
    products_to_show = products
    if filter_text:
        products_to_show = products[products["product_name"].str.contains(filter_text, case=False, na=False)]
    ...
    for _, product in products_to_show.iterrows():
        ...
        cols = st.columns([4.4, 1.0, 0.52, 0.60, 0.52])
        cols[0].markdown(f"<div class='compact-product-name'>{product['product_name']}</div>", unsafe_allow_html=True)
        cols[1].markdown(f"<div class='compact-price'>${float(product['unit_sale_price']):.2f}</div>", unsafe_allow_html=True)
    ...
    for pid, qty in cart.items():
        if int(qty) > 0 and pid in product_lookup.index:
            selected_total += float(product_lookup.loc[pid, "unit_sale_price"]) * int(qty)
    st.caption(f"{t('products_sold_count')}: {total_items} | {t('total_to_collect')}: ${selected_total:.2f}")
